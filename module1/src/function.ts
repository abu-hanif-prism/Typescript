...
let subtract = (a: number, b: number): number => a - b; // explicitly typed parameters and return type
console.log("Subtraction:", subtract(10, 5)); // Output: Subtraction: 5 
...
const poorUser = {
    ...
    getDonation: function(amount: number): void {
        this.balance += amount;
...
const sqrArray = arr1.map((elem: number): number => elem * elem); // using arrow function as callback
